...
namespace StarterAssets
{
    ...
    public class PlayerMovement : MonoBehaviour
    {
        ...
        private void Move()
        {
            ...
            float targetSpeed = PlayerInputController.Singleton.MoveState == MovementState.Run ? SprintSpeed : MoveSpeed;

            // a simplistic acceleration and deceleration designed to be easy to remove, replace, or iterate upon

            // note: Vector2's == operator uses approximation so is not floating point error prone, and is cheaper than magnitude
            // if there is no input, set the target speed to 0
            if (PlayerInputController.Singleton.InputMove == Vector2.zero) targetSpeed = 0.0f;
            ...
            Vector3 inputDirection = new Vector3(PlayerInputController.Singleton.InputMove.x, 0.0f, PlayerInputController.Singleton.InputMove.y).normalized;

            // note: Vector2's != operator uses approximation so is not floating point error prone, and is cheaper than magnitude
            // if there is a move input rotate player when the player is moving
            if (PlayerInputController.Singleton.InputMove != Vector2.zero)
            {
        ...
        private void JumpAndGravity()
        {
            ...
            else
            {
                ...
                PlayerInputController.Singleton.InputJump = false;
            }
